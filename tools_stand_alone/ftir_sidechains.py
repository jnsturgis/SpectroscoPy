...
def calc_spec_comp( x_values, posn, fwhh, fg ):
    """
    Calculate a compont
    """
...
def calc_resid_spectrum(resid, ph:float, d2o: bool, wn_info ):
    ...
    if data[PKA] != 0:
        base_form = calc_base(data[PKA],ph)
    ...
    for weight, freq, fwhh, fg in zip(
        ...
        my_spectrum[1] = my_spectrum[1] + weight * calc_spec_comp(my_spectrum[0], freq, fwhh, fg)
...
def sidechain(sequence, wn_range=(LOW_FREQ, HIGH_FREQ), **kwargs ):
    ...
    if not isinstance(wn_range, tuple) or len(wn_range) != 2:
        raise ValueError("wn_range must be a 2-tuple (start_freq, end_freq)")
    ...
    if not isinstance(options.get("res"), float ):
        raise ValueError("res (spectrum resolution) must be a float")
    if not isinstance(options.get("D2O"), bool ):
        raise ValueError("D2O must be a boolean")
    if not isinstance(options.get("addN"), bool ):
        raise ValueError("addN must be a boolean")
    if not isinstance(options.get("addC"), bool ):
        raise ValueError("addC must be a boolean")
