...
### 23.4 ✅ Scattering correction (2026-08-04)

`spectroscopy/processing/scattering.py`, 14 tests. It belongs *before*
unmixing, and the guide now shows why with numbers: on a synthetic turbid
sample the uncorrected fit returns **0.240 µg/mL nucleic acid against a true
0.030** — eight times too high, because a scattering background is not one of
the components and the fit spreads it across the ones it has. Corrected, the
same sample returns 0.030.

James's instinct to interpolate several backgrounds rather than assume
Rayleigh was right, and the margin is large. Fitting a **basis** of λ⁻ⁿ for
n = 4, 3, 2, 1, 0.5 recovers the background to a maximum error of **0.0048**;
a single fixed λ⁻⁴ manages **3.14** on the same data — 650 times worse. A
fixed exponent is a guess about particle size, and real suspensions are
polydisperse.

`from_references()` takes measured blanks instead, which is better again where
they exist: a suspension of the same particles without the chromophore
scatters the way the sample scatters, including whatever a power law cannot
express. Both routes are non-negative, since scattering adds signal.

The correction warns when the fit region still holds structure afterwards,
which is the one way it goes badly wrong — a region that is not
absorption-free lets the background eat a band. The first version of that
check tested for "mostly negative" and did not fire; a band inside the region
leaves a *structured* residual, not a one-sided one.

**The Bradford cross-validation** for §13.3 item 2 still needs samples
measured both ways.

**Real data.** Everything in §23 is validated against synthetic mixtures,
which proves the arithmetic and not that the methods survive a real
instrument. `Reference_Spectra_Wanted.md` is the measurement list that would
